...
class PG_SYNTHCITY_HYP(Serializable):
    ...
    @validate_arguments(config=dict(arbitrary_types_allowed=True))
    def fit(self, X_train: pd.DataFrame) -> Any:
        self.columns = X_train.columns
        self.generator_n_units_hidden = len(self.columns)
        self.discriminator_n_units_hidden = len(self.columns)
        ...
        self.model = TabularGAN(
            ...
            n_iter_print=self.generator_n_iter,
            device=self.device,
        )
        X_train_enc = self.model.encode(X_train)
        self.samples_per_teacher = int(len(X_train_enc) / self.num_teachers)
        ...
        while epsilon_hat < self.epsilon and it < self.max_iter:
            ...
            self.model.fit(X_train_enc, fake_labels_generator=fake_labels_generator, encoded=True)
